...
/// Initialise the given SUSPENDED state.
///
/// - Parameter state: The state to initialise.
void fsm_counterc_suspended_init(struct FSMCounterC_State_SUSPENDED * const state)
{
    state->check_transitions = (struct LLFSMState *(*)(const struct LLFSMachine *, const struct LLFSMState *))fsm_counterc_suspended_check_transitions;
    state->on_entry   = (void (*)(struct LLFSMachine *, struct LLFSMState *))fsm_counterc_suspended_on_entry;
    state->on_exit    = (void (*)(struct LLFSMachine *, struct LLFSMState *))fsm_counterc_suspended_on_exit;
    state->internal   = (void (*)(struct LLFSMachine *, struct LLFSMState *))fsm_counterc_suspended_internal;
    state->on_suspend = (void (*)(struct LLFSMachine *, struct LLFSMState *))fsm_counterc_suspended_on_suspend;
    state->on_resume  = (void (*)(struct LLFSMachine *, struct LLFSMState *))fsm_counterc_suspended_on_resume;
}

/// Check the validity of the given SUSPENDED state.
///
...
{
    (void)machine;
    return state->check_transitions == (struct LLFSMState *(*)(const struct LLFSMachine * const machine, const struct LLFSMState * const state))fsm_counterc_suspended_check_transitions &&
           state->on_entry   == (void (*)(struct LLFSMachine *, struct LLFSMState *))fsm_counterc_suspended_on_entry &&
           state->on_exit    == (void (*)(struct LLFSMachine *, struct LLFSMState *))fsm_counterc_suspended_on_exit &&
           state->internal   == (void (*)(struct LLFSMachine *, struct LLFSMState *))fsm_counterc_suspended_internal &&
           state->on_suspend == (void (*)(struct LLFSMachine *, struct LLFSMState *))fsm_counterc_suspended_on_suspend &&
